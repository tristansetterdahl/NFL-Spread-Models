...
clean_game <- function(name_col){
  #fox table has different abbreviations for some teams, need to find all and replace with this. oak changed to LV
  bad_abbs <- c('LAR', 'JAX', 'OAK')
  good_abbs <- c('LA', 'JAC', 'LV')
...
for(df in 1:length(all_teams_merged)){
  ...
  named <- paste0(tolower(abbs[df]), '_2020_box_data.csv')
...
big_clean <- function(all_teams_merged){
  ...
  for(i in 1:length(abbs)){
    ...
    curr_team$Date <- with_year(curr_team$Date, 2020)
...
for(df in 1:length(clean_all_merged)){
  ...
  named <- paste0(tolower(abbs[df]), '_2020_avgs.csv')
  write.csv(dat.fm, named, row.names = FALSE)
}
...
spread_dat <- read_csv('2020_nfl_spreads.csv')
spread_dat %>% head
...
write.csv(underdogs, '2020_underdog_avgs.csv', row.names = FALSE)
write.csv(favorites, '2020_favorites_avgs.csv', row.names = FALSE)

games_2020 %>% head
full_2020_data

full_2020_data_left <- left_join(games_2020, favorites, by = c('Date', 'Favorite', 'Underdog'))
full_2020_data <- left_join(full_2020_data_left, underdogs, by = c('Date', 'Underdog', 'Favorite'))

write.csv(full_2020_data, 'all_data_2020.csv')

full_2020_data %>% is.na %>% which
apply(is.na(full_2020_data), 2, which) 

full_2020_data$Fav.KRAvg[c(1, 8, 16, 20)]
full_2020_data$Fav.PRAvg[c(5, 14, 33)]
full_2020_data$Fav.PuntAvg[5]
full_2020_data$UD.FGPct[9]
full_2020_data$UD.XPPct[6]
full_2020_data$UD.KRAvg[c(2, 3, 6, 9, 12, 15, 16, 17)]
full_2020_data$UD.PRAvg[c(5, 7, 8, 11, 21, 26, 39)]

full_2020_data[,c('Date', 'Underdog', 'UD.FGPct')][9,]


all_teams_merged$CHI[,c('Date', 'FUML.turnovers')]
colnames(clean_all_merged$CHI)
