...
nfl_abb20 <- function(team_col){
  ...
  team_col_abbs <- abbs[match(team_col %>% str_sub(1, 13), team_names20 %>% str_sub(1,13))]
  return(team_col_abbs)
}

#creates columns to track each team's divisions for every game
nfl_divs20 <- function(df){
...
training_bets <- function(){
  #tables for weekly schedule 2020 on this website
  betting_2020_dat <- read_html('https://www.sportsoddshistory.com/nfl-game-season/?y=2020#1')
  betting_2020 <- betting_2020_dat %>% html_nodes('table')
  
  #scrapes tables week by week until the regular season is in one cohesive dataframe
  week = 1
  spread_dat_2020 <- tibble()
  for(i in 7:23){
    assign(paste0('betting_2020_wk', week), (betting_2020[[i]] %>% html_table())[1:19, 1:10] %>% as.data.frame() %>% drop_na())
    
    week = week + 1
    spread_dat_2020 <- rbind(spread_dat_2020, get(paste0('betting_2020_wk', week)))
  }
  
  #adding postseason to the dataframe
  post_20 <- (betting_2020[[24]] %>% html_table())[1:19, 1:11] %>% as.data.frame() %>% drop_na()
  post_20 <- post_20[,-1]
  colnames(post_20) <- colnames(spread_dat_2020)
  
  spread_dat_2020 <- rbind(spread_dat_2020, post_20)
  
  return(spread_dat_2020)
}
...
games_2020 <- training_bets()
games_2020$Favorite <- nfl_abb20(games_2020$Favorite)
games_2020$Underdog <- nfl_abb20(games_2020$Underdog)
games_2020.2 <- clean_scores(games_2020)
games_2020.2 <- nfl_divs20(games_2020.2)

##converting chr to factor cause didnt like when I tried inside the function. integrate this!
games_2020.2$Favorite %<>% as.factor()
games_2020.2$Underdog %<>% as.factor()
games_2020.2$OverUnder %<>% as.factor()
games_2020.2$SpreadWin %<>% as.factor()
games_2020.2$SpreadWinTeam %<>% as.factor()
games_2020.2$Winner %<>% as.factor()
games_2020.2$WinTeam %<>% as.factor()
games_2020.2$FavDiv %<>% as.factor()
games_2020.2$UDDiv %<>% as.factor()

games_2020 <- select(games_2020.2, c(Date, `Time (ET)`, Where, Favorite, FavDiv, Underdog, UDDiv, Spread, OULine,
                  FavScore, UDogScore, OT, Differential, OverUnder, SpreadWin, Winner, SpreadWinTeam, WinTeam))
write.csv(games_2020, "2020_nfl_spreads.csv", row.names = FALSE)

getwd()
games_2020.2 %>% head
games_2020 %>% head
games_2020.2$Winner[24]

nfc_east_ud <- games_2020.2 %>% filter(UDDiv == 'NFC East')




###SUPER GOOD VISUAL, KEEP DOING THIS
ggplot(games_2020.2, mapping = aes(x = FavDiv, fill = SpreadWin)) + geom_bar(position = 'fill')
